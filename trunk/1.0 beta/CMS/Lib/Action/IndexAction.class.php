...
class IndexAction extends BaseAction {
	public function index(){
		...
		$iarticles=$dat->table(C('DB_PREFIX').'Article as a,'.C('DB_PREFIX').'Menu as b')->findAll('a.status>0 AND b.menuId=a.menuId AND b.indexMenu=1','a.menuId,b.menuId,a.aid,a.uid,a.title,a.titlecolor,a.cTime,a.status,b.indexMenu','cTime desc',100);
		//dump($iarticles);

		//$iarticles=$DAO->findall('status>0 AND (select indexMenu from'.C('DB_PREFIX').'menu)','aid,uid,menuId,title,mTime',100);
