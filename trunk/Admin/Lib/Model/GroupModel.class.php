...
{
    $table = C("DB_PREFIX").'access';
    $rs = $this->query('select a.nodeId,a.groupId,b.id,b.title,b.name from '.$table.' as a ,'.C("DB_PREFIX").'node as b where a.nodeId=b.id and  b.pid='.$appId.' and a.groupId='.$groupId.' ',false);
    return $rs;
	//return $this->rsToVo($rs);
}
...
{
    $table = C("DB_PREFIX").'groupuser';
    $rs = $this->query('select a.userId,a.groupId,b.uid,b.username from '.$table.' as a ,'.C("DB_PREFIX").'user as b where a.userId=b.uid and  a.groupId='.$groupId.' ',false);
    return $rs;
	//return $this->rsToVo($rs);
}
...
function setGroupUsers($groupId,$userIdList) 
{
    ...
    $where = 'a.id ='.$groupId.' AND b.uid in('.$id.')';
    $rs = $this->execute('INSERT INTO '.C("DB_PREFIX").'groupuser (groupId,userId) SELECT a.id, b.uid FROM '.C("DB_PREFIX").'group a, '.C("DB_PREFIX").'user b WHERE '.$where);
    if($result===false) {
        return false;
    }else {
        return true;
